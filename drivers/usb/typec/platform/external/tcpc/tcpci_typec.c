...
static inline void typec_source_attached_entry(struct tcpc_device *tcpc)
{
	...
	typec_set_plug_orient(
		tcpc, TYPEC_CC_PULL(tcpc->typec_local_rp_level, TYPEC_CC_RP),
					     ...
					     bool vbus_valid)
{
	if (vbus_valid) {
		...
		tcpci_sink_vbus(tcpc, TCP_VBUS_CTRL_TYPEC, TCPC_VBUS_SINK_0V,
				0);
		tcpci_report_power_control(tcpc, false);
	}
...
static inline void typec_attach_wait_entry(struct tcpc_device *tcpc)
{
	...
	if (as_sink) {
		...
		tcpci_set_cc(tcpc, TYPEC_CC_PULL(tcpc->typec_local_rp_level,
						 TYPEC_CC_RP));
...
static inline void typec_handle_pd_wait_bc12(struct tcpc_device *tcpc)
{
	...
	TYPEC_INFO("type=%d, ret,chg_type=%d,%d, count=%d\n", type, ret,
		   val.intval, tcpc->pd_wait_bc12_count);
