...
int tcpm_typec_set_usb_sink_curr(struct tcpc_device *tcpc, int curr)
{
	...
	if (force_sink_vbus) {
		tcpci_sink_vbus(tcpc, TCP_VBUS_CTRL_TYPEC, TCPC_VBUS_SINK_5V,
				-1);
	}
	tcpci_unlock_typec(tcpc);
				 ...
				 struct tcpm_power_cap_list *cap_list)
{
	...
	for (i = 0; i < cap_list->nr; i++) {
		if (!tcpm_extract_power_cap_val(cap->pdos[i],
						&cap_list->cap_val[i]))
			return false;
	}
				 ...
				 struct tcpm_power_cap_val *cap_val)
{
	...
	if (!tcpm_extract_power_cap_val(pe_data->remote_src_cap.pdos[sel],
					cap_val))
...
static inline int
tcpm_put_tcp_dpm_event_cb(struct tcpc_device *tcpc, struct tcp_dpm_event *event,
...
#ifdef CONFIG_USB_PD_BLOCK_TCPM
	if (cb_data == NULL) {
		return tcpm_put_tcp_dpm_event_bk(tcpc, event, tout_ms, data,
						 size);
	}
#endif /* CONFIG_USB_PD_BLOCK_TCPM */
...
static inline int
tcpm_update_bat_status_soc_no_mutex(struct tcpc_device *tcpc,
				    enum pd_battery_reference ref,
...
static inline int tcpm_dpm_wait_bk_event(struct pd_port *pd_port,
					 uint32_t tout_ms)
{
				     ...
				     uint8_t size)
{
	...
	while (1) {
		ret = __tcpm_put_tcp_dpm_event_bk(tcpc, event, tout_ms, data,
						  size);
