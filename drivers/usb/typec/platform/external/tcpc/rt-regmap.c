				 ...
				 int bytes, const void *src, bool asyn)
{
	...
	for (i = rio.index, j = rio.offset, count = 0;
	     ...
	     i++, j = 0, count += size) {
		rm = rd->props.rm[i];
		size = (bytes - count) <= (rm->size - j) ? (bytes - count) :
							   (rm->size - j);
		if ((rm->reg_type & RT_REG_TYPE_MASK) == RT_VOLATILE) {
			...
			ret = rd->rt_block_write[blk_index](rd, rm, size, wdata,
							    count, i, j);
		}
		if (ret < 0) {
			dev_notice(&rd->dev,
...
static int rt_regmap_cache_init(struct rt_regmap_device *rd)
{
	...
	if (rd->props.group == NULL) {
		rd->props.group = devm_kzalloc(
			&rd->dev, sizeof(*rd->props.group) * 2, GFP_KERNEL);
		if (!rd->props.group) {
			ret = -ENOMEM;
			goto out;
		}
...
#define erro_printf(rd, fmt, ...)                                              \
	do {                                                                   \
		...
		ret = snprintf(rd->err_msg + len, ERR_MSG_SIZE - len, fmt,     \
			       ##__VA_ARGS__);                                 \
		rd->error_occurred = 1;                                        \
		up(&rd->semaphore);                                            \
...
static int general_read(struct seq_file *seq_file, void *_data)
{
	...
	case RT_DBG_DATA:
		...
		if (rd->dbg_data.rio.index < 0)
			...
			ret = rd->regmap_ops.regmap_block_read(
				rd, rd->dbg_data.reg_addr, size, rd->regval);
		up(&rd->semaphore);
		if (ret < 0) {
			erro_printf(rd, "%s block read fail(%d) @ 0x%02x\n",
				 ...
				 struct dentry *dir)
{
	...
	for (i = 0; i < rd->props.register_num; i++) {
		...
		rd->rt_reg_file[i] = devm_kzalloc(
			&rd->dev, sizeof(*rd->rt_reg_file[i]), GFP_KERNEL);
		rd->reg_st[i] = devm_kzalloc(&rd->dev, sizeof(*rd->reg_st[i]),
					     GFP_KERNEL);
		...
		rd->reg_st[i]->info = rd;
		rd->reg_st[i]->id = i;
		rd->rt_reg_file[i] = debugfs_create_file(
			buf, 0444, dir, rd->reg_st[i], &eachreg_ops);
		if (!rd->rt_reg_file[i])
			return -EINVAL;
	}
