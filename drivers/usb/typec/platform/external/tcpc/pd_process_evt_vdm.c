...
#define VDM_CMD_REACTION(cmd, init, req, flags)                                \
	{                                                                      \
		.vdm_cmd = cmd, .vdm_init_state = init,                        \
		.vdm_request_state = req, .vdm_cmd_flags = flags,              \
	}
		     ...
		     const struct vdm_state_transition *state_transition)
{
	...
	if (vdm_cmdt == CMDT_INIT) { /* Recv */
		if (!vdm_is_state_transition_available(pd_port, true,
						       state_transition))
			return false;
	...
	/* Send */
	if (!vdm_is_state_transition_available(pd_port, false,
					       state_transition))
		return false;
				      ...
				      uint8_t transit_type)
{
	...
	for (i = 0; i < nr_transition; i++) {
		...
		if (check_tx) {
			...
			return pd_vdm_state_transit(pd_port, vdm_cmdt,
						    state_transition);
		}
	}
...
{
	return pd_make_vdm_state_transit(pd_port,
					 VDM_STATE_TRANSIT_SOP_PRIME_CMD);
}
...
static inline void pd_parse_tcp_dpm_evt_dp_status(struct pd_port *pd_port)
{
	...
	dp_data->local_status = tcpc_update_bits(
		dp_data->local_status, dp_data_tcp->val, dp_data_tcp->mask);
}
...
static inline void pd_parse_tcp_dpm_evt_uvdm(struct pd_port *pd_port)
{
	...
	memcpy(pd_port->uvdm_data, vdm_data->vdos,
