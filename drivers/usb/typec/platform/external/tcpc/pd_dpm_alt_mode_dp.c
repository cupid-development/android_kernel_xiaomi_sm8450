...
#define DP_SELECT_CONNECTED(b)                                                 \
	((b == DPSTS_DFP_D_CONNECTED) ? DPSTS_UFP_D_CONNECTED :                \
				    ...
				    struct svdm_svid_data *svid_data, bool ack)
{
	...
	pd_port->mode_obj_pos =
		dp_dfp_u_select_mode(pd_port, dp_data, svid_data);
...
bool dp_dfp_u_notify_dp_status_update(struct pd_port *pd_port, bool ack)
{
	...
	if (oper_mode) {
		tcpci_dp_notify_status_update_done(tcpc, dp_data->remote_status,
						   ack);
	} else {
			...
			struct svdm_svid_data *svid_data)
{
	...
	svid_data->local_mode.mode_vdo[0] =
		VDO_MODE_DP(ufp_d_pin_cap, dfp_d_pin_cap, usb2, receptacle,
			    signal,
