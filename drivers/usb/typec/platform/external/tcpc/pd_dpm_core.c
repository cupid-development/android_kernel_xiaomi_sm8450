					...
					uint8_t charging_policy)
{
	...
	dpm_build_sink_pdo_info(&sink_pdo_info, DPM_PDO_TYPE_APDO,
				pd_port->request_v_apdo,
				pd_port->request_i_apdo);
				       ...
				       uint8_t charging_policy)
{
	...
	for (i = 0; i < snk_cap->nr; i++) {
		...
		find_cap = dpm_find_match_req_info(req_info, &sink_pdo_info,
		...
		if (find_cap) {
			...
			DPM_DBG("Find SrcCap%d(%s):%d mw\n", req_info->pos,
				req_info->mismatch ? "Mismatch" : "Match",
				max_uw / 1000);
			      ...
			      struct tcp_dpm_pd_request *pd_req)
{
	...
	find_cap =
		dpm_find_match_req_info(&req_info, &sink_pdo_info, src_cap->nr,
					src_cap->pdos, -1, charging_policy);
...
void pd_dpm_snk_standby_power(struct pd_port *pd_port)
{
	...
	int ma = -1;
	int standby_curr =
		2500000 / max(pd_port->request_v, pd_port->request_v_new);
	...
	if (ma >= 0) {
		tcpci_sink_vbus(pd_port->tcpc, type, pd_port->request_v_new,
				ma);
	}
#else
...
void pd_dpm_snk_transition_power(struct pd_port *pd_port)
{
	...
	if (pd_port->request_apdo != pd_port->request_apdo_new) {
		...
		pd_dpm_start_pps_request_thread(pd_port,
						pd_port->request_apdo_new);
	}
#endif /* CONFIG_USB_PD_REV30_PPS_SINK */
}
...
void pd_dpm_src_transition_power(struct pd_port *pd_port)
{
	...
	if (pd_port->request_v > pd_port->request_v_new) {
		...
		tcpci_enable_force_discharge(pd_port->tcpc, true,
...
void pd_dpm_dfp_inform_exit_mode(struct pd_port *pd_port)
{
	...
	dpm_dfp_update_svid_data_exit_mode(pd_port, expected_svid,
					   expected_ops);
...
void pd_dpm_dfp_inform_uvdm(struct pd_port *pd_port, bool ack)
{
	...
	if (svid_data) {
		if (svid_data->ops->dfp_notify_uvdm)
			svid_data->ops->dfp_notify_uvdm(pd_port, svid_data,
							ack);
	}
...
{
	/* iSnkSwapStdby : 2.5mA */
	tcpci_sink_vbus(pd_port->tcpc, TCP_VBUS_CTRL_PR_SWAP, TCPC_VBUS_SINK_0V,
			0);
}
...
int pd_dpm_send_battery_status(struct pd_port *pd_port)
{
	...
	return pd_send_sop_data_msg(pd_port, PD_DATA_BAT_STATUS, PD_BSDO_SIZE,
				    bsdo);
}
...
int pd_dpm_send_country_info(struct pd_port *pd_port)
{
	...
	for (i = 0; i < pd_port->country_nr; i++) {
		if (ca[i].code == cc) {
			cidb_size += ca[i].len;
			memcpy(cidb.country_special_data, ca[i].data,
			       ca[i].len);
			break;
		}
	}

	return pd_send_sop_ext_msg(pd_port, PD_EXT_COUNTRY_INFO, cidb_size,
				   &cidb);
}
...
int pd_dpm_notify_pe_startup(struct pd_port *pd_port)
{
	...
	reactions |=
		DPM_REACTION_ATTEMPT_GET_FLAG | DPM_REACTION_REQUEST_PR_SWAP;
#else
...
int pd_dpm_core_init(struct pd_port *pd_port)
{
	...
	atomic_set(&pd_port->pps_request, false);
	pd_port->pps_request_task = kthread_run(
		pps_request_thread_fn, tcpc, "pps_request_%s", tcpc->desc.name);
#endif /* CONFIG_USB_PD_REV30 */
