...
static int rt1711_tcpc_init(struct tcpc_device *tcpc, bool sw_reset)
{
	...
	rt1711_i2c_write16(tcpc, RT1711H_REG_DRP_DUTY_CTRL,
			   TCPC_NORMAL_RP_DUTY);
	...
	rt1711_i2c_write8(tcpc, RT1711H_REG_PHY_CTRL1,
			  RT1711H_REG_PHY_CTRL1_SET(retry_discard_old, 7, 0,
						    1));
...
static int rt1711_tcpcdev_init(struct rt1711_chip *chip, struct device *dev)
{
	...
	if (of_property_read_u32(np, "rt-tcpc,notifier_supply_num", &val) >=
	    0) {
		if (val < 0)
	...
	if (of_property_read_string(np, "rt-tcpc,name", (char const **)&name) <
	    0) {
...
static void rt1711_shutdown(struct i2c_client *client)
{
	...
	if (chip != NULL) {
		...
		if (chip->chip_id == HUSB311_DID) {
			i2c_smbus_write_byte_data(client, RT1711H_REG_BMC_CTRL,
						  0x00);
		}
	}
}
...
#ifdef CONFIG_PM_RUNTIME
		SET_RUNTIME_PM_OPS(rt1711_pm_suspend_runtime,
				   rt1711_pm_resume_runtime, NULL)
#endif /* #ifdef CONFIG_PM_RUNTIME */
};
