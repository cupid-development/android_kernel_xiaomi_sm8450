...
#define TCPC_DEVICE_ATTR(_name, _mode)                                         \
	{                                                                      \
		...
		.show = tcpc_show_property, .store = tcpc_store_property,      \
	}
...
static struct device_attribute tcpc_device_attributes[] = {
	TCPC_DEVICE_ATTR(role_def, 0444), TCPC_DEVICE_ATTR(rp_lvl, 0444),
	TCPC_DEVICE_ATTR(pd_test, 0664),  TCPC_DEVICE_ATTR(info, 0444),
	TCPC_DEVICE_ATTR(timer, 0664),	  TCPC_DEVICE_ATTR(caps_info, 0444),
				  ...
				  struct device_attribute *attr, char *buf)
{
	...
	case TCPC_DESC_CAP_INFO:
		...
		for (i = 0; i < pd_port->local_src_cap.nr; i++) {
			tcpm_extract_power_cap_val(
				pd_port->local_src_cap.pdos[i], &cap);
		...
		for (i = 0; i < pd_port->local_snk_cap.nr; i++) {
			tcpm_extract_power_cap_val(
				pd_port->local_snk_cap.pdos[i], &cap);
			snprintf(buf + strlen(buf), 256, "%d %d %d %d\n",
				 cap.type, cap.min_mv, cap.max_mv, cap.ma);
		}
		snprintf(buf + strlen(buf), 256, "%s\n",
		...
		for (i = 0; i < pe_data->remote_src_cap.nr; i++) {
			tcpm_extract_power_cap_val(
				pe_data->remote_src_cap.pdos[i], &cap);
			snprintf(buf + strlen(buf), 256, "%d %d %d %d\n",
				 cap.type, cap.min_mv, cap.max_mv, cap.ma);
		}
		snprintf(buf + strlen(buf), 256, "%s\n",
		...
		for (i = 0; i < pe_data->remote_snk_cap.nr; i++) {
			tcpm_extract_power_cap_val(
				pe_data->remote_snk_cap.pdos[i], &cap);
			snprintf(buf + strlen(buf), 256, "%d %d %d %d\n",
				 cap.type, cap.min_mv, cap.max_mv, cap.ma);
		}
		break;
	...
	case TCPC_DESC_PD_TEST:
		snprintf(buf, 256, "%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n",
			 ...
			 "4: soft reset", "5: hard reset", "6: get_src_cap",
			 "7: get_sink_cap", "8: discover_id",
			 "9: discover_cable");
		break;
	case TCPC_DESC_INFO:
...
{
	struct device *dev =
		class_find_device(tcpc_class, NULL, (const void *)name,
...
int tcpc_device_irq_enable(struct tcpc_device *tcpc)
{
	...
	schedule_delayed_work(&tcpc->event_init_work,
				       ...
				       struct notifier_block *nb, uint8_t idx)
{
	...
	retval = srcu_notifier_chain_register(tcp_dev->evt_nh + idx,
					      &nb_wrapper->stub_nb);
	if (retval < 0) {
	...
	retval =
		__add_wrapper_to_managed_res_list(tcp_dev, nb_wrapper, nb, idx);
	if (retval < 0)
			      ...
			      struct notifier_block *nb, uint8_t flags)
{
	...
	if (__is_mulit_bits_set(flags)) {
		for (i = 0; i < TCP_NOTIFY_IDX_NR; i++) {
			if (flags & (1 << i)) {
				ret = __register_tcp_dev_notifier(tcp_dev, nb,
								  i);
				if (ret < 0)
					return ret;
			}
