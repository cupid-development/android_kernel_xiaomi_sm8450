...
static inline void start_usb_host(struct rt_pd_manager_data *rpmd)
{
	...
	val.intval = 1;
	extcon_set_property(rpmd->extcon, EXTCON_USB_HOST, EXTCON_PROP_USB_SS,
			    val);
...
static void usb_dwork_handler(struct work_struct *work)
{
	...
	dev_info(rpmd->dev, "%s %s otg_conn_therm_flag: %d\n", __func__,
		 dr_names[usb_dr], rpmd->otg_conn_therm_flag);
	...
	case DR_DEVICE:
		...
		if (pd_active) {
			...
			ret = xm_battmngr_read_iio_prop(g_battmngr_iio,
							MAIN_CHG,
							MAIN_CHARGER_USB_TYPE,
							&val.intval);
		}
		dev_info(rpmd->dev, "%s polling_cnt = %d, ret = %d type = %d\n",
			 __func__, ++rpmd->usb_type_polling_cnt, ret,
			 val.intval);
...
static void pd_sink_set_vol_and_cur(struct rt_pd_manager_data *rpmd, int mv,
				    int ma, uint8_t type)
{
	...
	xm_battmngr_write_iio_prop(g_battmngr_iio, PD_PHY, PD_ACTIVE,
				   val.intval);
	...
	val.intval = ma * 1000;
	xm_battmngr_write_iio_prop(g_battmngr_iio, PD_PHY, PD_CURRENT_MAX,
				   val.intval);
}
				...
				void *data)
{
	...
	case TCP_NOTIFY_TYPEC_STATE:
		...
		if (old_state == TYPEC_UNATTACHED &&
			...
			if ((noti->typec_state.rp_level >=
			     TYPEC_CC_VOLT_SNK_DFT) &&
			    (noti->typec_state.rp_level <=
			     (TYPEC_CC_VOLT_SNK_3_0 + 1)))
			...
			if (!rpmd->batt_psy) {
				...
				power_supply_get_property(
					rpmd->batt_psy,
					POWER_SUPPLY_PROP_CAPACITY, &val1);
			}
			t2 = t2 - t1;
		...
		if (new_state == TYPEC_UNATTACHED) {
			...
			xm_battmngr_write_iio_prop(
				g_battmngr_iio, PD_PHY, PD_TYPEC_ACCESSORY_MODE,
				rpmd->partner_desc.accessory);
			dev_err(rpmd->dev, "%s TYPEC_ACCESSORY_MODE(%d)\n",
				__func__, rpmd->partner_desc.accessory);
			rpmd->partner = typec_register_partner(
				rpmd->typec_port, &rpmd->partner_desc);
	...
	case TCP_NOTIFY_PD_STATE:
		...
		case PD_CONNECT_PE_READY_SNK_PD30:
			...
			val.intval =
				ret & DPM_FLAGS_PARTNER_USB_SUSPEND ? 1 : 0;
			xm_battmngr_write_iio_prop(g_battmngr_iio, PD_PHY,
						   PD_USB_SUSPEND_SUPPORTED,
						   val.intval);
...
static int rt_pd_manager_probe(struct platform_device *pdev)
{
	...
	if (!g_battmngr || !g_tcpc_rt1711h || !g_xm_pd_adapter) {
		dev_err(&pdev->dev,
			"%s: g_battmngr or g_tcpc_rt1711h or g_xm_pd_adapter not ready, defer\n",
			__func__);
		ret = -EPROBE_DEFER;
		msleep(100);
