...
static const struct wl2866d_map wl2866d_on_config[] = {
	{ 0x03, 0x54 }, { 0x04, 0x64 }, { 0x05, 0x30 }, { 0x06, 0x30 },
	{ 0x0E, 0x0F }, { 0x0E, 0x00 }, { 0x02, 0x8F }, { 0x02, 0x00 },
};
...
static int wl2866d_i2c_write(struct wl2866d_chip *chip, unsigned char reg_addr,
			     unsigned char reg_data)
{
	...
	while (cnt < AW_I2C_RETRIES) {
		ret = i2c_smbus_write_byte_data(chip->client, reg_addr,
						reg_data);
		if (ret < 0) {
...
static int wl2866d_i2c_read(struct wl2866d_chip *chip, unsigned char reg_addr,
			    unsigned char *reg_data)
{
	int ret = -1;
	unsigned char cnt = 0;
...
int wl2866d_camera_power_up(int out_iotype, int out_delay)
{
	...
	case OUT_DVDD1:
		...
		ret = wl2866d_i2c_write(camera_chip,
					wl2866d_on_config[OUT_DVDD1].reg,
					output_vol.dvdd1); //bit0
		if (ret < 0) {
			mutex_unlock(&wl2866d_mutex);
		...
		ret = wl2866d_i2c_read(camera_chip,
	...
	case OUT_DVDD2:
		...
		mutex_lock(&wl2866d_mutex);
		ret = wl2866d_i2c_write(camera_chip,
					wl2866d_on_config[OUT_DVDD2].reg,
					output_vol.dvdd2); //bit1
		if (ret < 0) {
			mutex_unlock(&wl2866d_mutex);
	...
	case OUT_AVDD1:
		...
		mutex_lock(&wl2866d_mutex);
		ret = wl2866d_i2c_write(camera_chip,
					wl2866d_on_config[OUT_AVDD1].reg,
					output_vol.avdd1); //bit2
		if (ret < 0) {
			mutex_unlock(&wl2866d_mutex);
	...
	case OUT_AVDD2:
		...
		mutex_lock(&wl2866d_mutex);
		ret = wl2866d_i2c_write(camera_chip,
					wl2866d_on_config[OUT_AVDD2].reg,
					output_vol.avdd2); //bit3
		if (ret < 0) {
			mutex_unlock(&wl2866d_mutex);
			pr_err("wl2866d set avdd1 failed\n");
			break;
					...
					const char *buf, size_t count)
{
	...
	ret = wl2866d_i2c_write(chip, wl2866d_on_config[VOL_ENABLE].reg,
				reg_val);
	if (ret < 0) {
		pr_err("wl2866d set enable failed\n");
		return ret;
				       ...
				       const char *buf, size_t count)
{
	...
	ret = wl2866d_i2c_write(chip, wl2866d_on_config[OUT_AVDD2].reg,
				reg_val);
	if (ret < 0) {
				       ...
				       const char *buf, size_t count)
{
	...
	ret = wl2866d_i2c_write(chip, wl2866d_on_config[OUT_AVDD1].reg,
				reg_val);
	if (ret < 0) {
				       ...
				       const char *buf, size_t count)
{
	...
	ret = wl2866d_i2c_write(chip, wl2866d_on_config[OUT_DVDD1].reg,
				reg_val);
	if (ret < 0) {
...
static DEVICE_ATTR(vol_enable, 0664, wl2866d_vol_enable_show,
		   wl2866d_vol_enable_store);
static DEVICE_ATTR(out_avdd2, 0664, wl2866d_out_avdd2_show,
		   wl2866d_out_avdd2_store);
static DEVICE_ATTR(out_avdd1, 0664, wl2866d_out_avdd1_show,
		   wl2866d_out_avdd1_store);
static DEVICE_ATTR(out_dvdd2, 0664, wl2866d_out_dvdd2_show,
		   wl2866d_out_dvdd2_store);
static DEVICE_ATTR(out_dvdd1, 0664, wl2866d_out_dvdd1_show,
		   wl2866d_out_dvdd1_store);

static struct attribute *wl2866d_attributes[] = {
	&dev_attr_out_dvdd1.attr,  &dev_attr_out_dvdd2.attr,
	&dev_attr_out_avdd1.attr,  &dev_attr_out_avdd2.attr,
	&dev_attr_vol_enable.attr, NULL
};
...
static int set_init_voltage(struct wl2866d_chip *chip)
{
	...
	for (i = 0; i <= LAST_POWER_CH; i++) {
		ret = wl2866d_i2c_write(chip, wl2866d_on_config[i].reg,
					wl2866d_on_config[i].value);
		if (ret < 0) {
	...
	ret = wl2866d_i2c_write(chip, wl2866d_on_config[VOL_DISABLE].reg,
				wl2866d_on_config[VOL_DISABLE].value);
	if (ret < 0) {
		pr_err("wl2866d  dischager function enable failed\n");
		return ret;
	}
	//enable dischager function
	ret = wl2866d_i2c_write(chip, wl2866d_on_config[DISCHARGE_ENABLE].reg,
...
int wl2866d_camera_power_up_all(void)
{
	...
	for (i = 0; i <= POWER_UP_ALL_INDEX; i++) {
		ret = wl2866d_i2c_write(camera_chip, wl2866d_on_config[i].reg,
					wl2866d_on_config[i].value);
		if (ret < 0) {
			pr_err("wl2866d init voltage failed\n");
			return ret;
...
void wl2866d_print_reg(struct wl2866d_chip *chip)
{
	...
	for (i = 0; i < ARRAY_SIZE(wl2866d_on_config); i++) {
		...
		pr_err("%s:wl2866d info is reg %d, value %d\n", __func__,
		       wl2866d_on_config[i].reg, reg_val);
	}
}
...
static int wl2866d_init(struct wl2866d_chip *chip)
{
	...
	chip->en_gpio = of_get_named_gpio(chip->dev->of_node, "en-gpio", 0);
...
unset_vin1:
	ret = regulator_set_voltage(chip->vin1, 0, VIN1_1P35_VOL_MAX);
	if (ret)
		dev_err(chip->dev, "Unable to set (0) voltage for vin1:%d\n",
			ret);
...
static int wl2866d_voltage_init(struct wl2866d_chip *chip)
{
	...
	ret = of_property_read_u8(chip->dev->of_node, "avdd1",
				  &output_vol.avdd1);
	...
	ret = of_property_read_u8(chip->dev->of_node, "avdd2",
				  &output_vol.avdd2);
	...
	ret = of_property_read_u8(chip->dev->of_node, "dvdd1",
				  &output_vol.dvdd1);
	...
	ret = of_property_read_u8(chip->dev->of_node, "dvdd2",
				  &output_vol.dvdd2);
	...
	pr_err("wl2866d output voltage, avdd1:0x%x, avdd2:0x%x, dvdd1:0x%x, dvdd2:0x%x\n",
	       output_vol.avdd1, output_vol.avdd2, output_vol.dvdd1,
	       output_vol.dvdd2);
			 ...
			 const struct i2c_device_id *id)
{
	...
	chip = devm_kzalloc(&client->dev, sizeof(struct wl2866d_chip),
			    GFP_KERNEL);
	if (!chip) {
		ret = -ENOMEM;
		goto err_mem;
	...
	ret = sysfs_create_group(&client->dev.kobj, &wl2866d_attribute_group);
	if (ret < 0) {
...
static const struct dev_pm_ops wl2866d_pm_ops = { SET_SYSTEM_SLEEP_PM_OPS(
	wl2866d_suspend, wl2866d_resume) };
