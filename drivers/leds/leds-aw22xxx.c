...
static char aw22xxx_imax_name[][AW22XXX_IMAX_NAME_MAX] = {
	{ "AW22XXX_IMAX_2mA" },	 { "AW22XXX_IMAX_3mA" },
	{ "AW22XXX_IMAX_4mA" },	 { "AW22XXX_IMAX_6mA" },
	{ "AW22XXX_IMAX_9mA" },	 { "AW22XXX_IMAX_10mA" },
	{ "AW22XXX_IMAX_15mA" }, { "AW22XXX_IMAX_20mA" },
	{ "AW22XXX_IMAX_30mA" }, { "AW22XXX_IMAX_40mA" },
	{ "AW22XXX_IMAX_45mA" }, { "AW22XXX_IMAX_60mA" },
	{ "AW22XXX_IMAX_75mA" },
};
 ...
 *
 ******************************************************/
static int aw22xxx_i2c_write(struct aw22xxx *aw22xxx, unsigned char reg_addr,
			     unsigned char reg_data)
{
	...
	while (cnt < AW_I2C_RETRIES) {
		ret = i2c_smbus_write_byte_data(aw22xxx->i2c, reg_addr,
						reg_data);
		if (ret < 0) {
...
static int aw22xxx_i2c_read(struct aw22xxx *aw22xxx, unsigned char reg_addr,
			    unsigned char *reg_data)
{
	int ret = -1;
	unsigned char cnt = 0;
...
static int aw22xxx_i2c_write_bits(struct aw22xxx *aw22xxx,
				  unsigned char reg_addr, unsigned char mask,
				  unsigned char reg_data)
{
...
static int aw22xxx_i2c_writes(struct aw22xxx *aw22xxx, unsigned char reg_addr,
...
static int aw22xxx_sys_init(struct aw22xxx *aw22xxx)
{
	...
	for (i = 0; i < sizeof(aw22xxx_init_code); i += 2) {
		aw22xxx_i2c_write(aw22xxx, aw22xxx_init_code[i],
				  aw22xxx_init_code[i + 1]);
	}
	return 0;
}
...
{
	if (flag) {
		aw22xxx_i2c_write_bits(aw22xxx, REG_GCR, BIT_GCR_CHIPEN_MASK,
...
static int aw22xxx_mcu_reset(struct aw22xxx *aw22xxx, bool flag)
{
	if (flag) {
		aw22xxx_i2c_write_bits(aw22xxx, REG_MCUCTR,
				       BIT_MCUCTR_MCU_RESET_MASK,
				       BIT_MCUCTR_MCU_RESET_ENABLE);
	} else {
		aw22xxx_i2c_write_bits(aw22xxx, REG_MCUCTR,
				       BIT_MCUCTR_MCU_RESET_MASK,
				       BIT_MCUCTR_MCU_RESET_DISABLE);
	}
	return 0;
}
...
static int aw22xxx_mcu_enable(struct aw22xxx *aw22xxx, bool flag)
{
	if (flag) {
		aw22xxx_i2c_write_bits(aw22xxx, REG_MCUCTR,
				       BIT_MCUCTR_MCU_WORK_MASK,
				       BIT_MCUCTR_MCU_WORK_ENABLE);
	} else {
		aw22xxx_i2c_write_bits(aw22xxx, REG_MCUCTR,
				       BIT_MCUCTR_MCU_WORK_MASK,
				       BIT_MCUCTR_MCU_WORK_DISABLE);
	}
	return 0;
}
...
{
	aw22xxx_i2c_write(aw22xxx, REG_ADDR1,
			  (unsigned char)((addr >> 0) & 0xff));
	aw22xxx_i2c_write(aw22xxx, REG_ADDR2,
...
static void aw22xxx_brightness_work(struct work_struct *work)
{
	struct aw22xxx *aw22xxx =
		container_of(work, struct aw22xxx, brightness_work);
...
static void aw22xxx_cfg_loaded(const struct firmware *cont, void *context)
{
	...
	pr_info("%s: loaded %s - size: %zu\n", __func__,
		aw22xxx_cfg_name[aw22xxx->effect], cont ? cont->size : 0);
	...
	for (i = 0; i < cont->size; i += 2) {
		...
		if (aw22xxx->cfg == 1) {
			aw22xxx_i2c_write(aw22xxx, *(cont->data + i),
					  *(cont->data + i + 1));
								 ...
								 (8 *
								  (2 -
								   reg_addr %
									   3))) &
								0xff);
					aw22xxx_i2c_write(aw22xxx,
							  *(cont->data + i),
							  reg_val);
					pr_debug(
						"%s: addr:0x%02x, data:0x%02x\n",
						__func__, *(cont->data + i),
						reg_val);
				} else if (reg_addr > 0x2b && reg_addr < 0xaa) {
					if ((reg_addr == 0x2f + cnt * 7) ||
					    (reg_addr == 0x30 + cnt * 7) ||
					    (reg_addr == 0x2e + cnt * 7) ||
					    (reg_addr == 0x31 + cnt * 7)) {
						reg_val = (aw22xxx->frq < 64) ?
								  aw22xxx->frq :
								  64;
						pr_debug(
							"%s: addr:0x%02x, data:0x%02x\n",
...
static int aw22xxx_cfg_update(struct aw22xxx *aw22xxx)
{
	...
	if (aw22xxx->effect <
	    (sizeof(aw22xxx_cfg_name) / AW22XXX_CFG_NAME_MAX)) {
	...
	rc = request_firmware_nowait(THIS_MODULE, FW_ACTION_HOTPLUG,
				     aw22xxx_cfg_name[aw22xxx->effect],
				     aw22xxx->dev, GFP_KERNEL, aw22xxx,
				     aw22xxx_cfg_loaded);
	if (rc) {
		pr_err("%s: load of '%s' failed: %d\n", __func__,
		       aw22xxx_cfg_name[aw22xxx->effect], rc);
		mutex_unlock(&aw22xxx->cfg_lock);
	}
				    ...
				    struct aw22xxx_container *aw22xxx_fw)
{
	...
	for (i = 0; i < aw22xxx_fw->len; i += tmp_len) {
		...
		aw22xxx_i2c_writes(aw22xxx, 0x23, &aw22xxx_fw->data[i],
				   tmp_len);
...
static void aw22xxx_fw_loaded(const struct firmware *cont, void *context)
{
	...
	aw22xxx_fw->version =
		(cont->data[0 + shift] << 24) | (cont->data[1 + shift] << 16) |
...
{
	unsigned char led;
	uint16_t ref[] = { 0,	 64,   128,  192,  256,	 320,  384,  448,
				 ...
				 struct device_attribute *attr, char *buf)
{
	...
	for (i = 0; i < sizeof(aw22xxx_imax_name) / AW22XXX_IMAX_NAME_MAX;
	     i++) {
	...
	len += snprintf(buf + len, PAGE_SIZE - len,
			...
			aw22xxx_imax_name[aw22xxx->imax]);
...
static ssize_t aw22xxx_frq_store(struct device *dev,
				 struct device_attribute *attr, const char *buf,
				 size_t len)
{
	uint16_t databuf[1];
...
static ssize_t aw22xxx_frq_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	ssize_t len = 0;
	struct led_classdev *led_cdev = dev_get_drvdata(dev);
...
static DEVICE_ATTR(hwen, S_IWUSR | S_IRUGO, aw22xxx_hwen_show,
		   aw22xxx_hwen_store);
...
static DEVICE_ATTR(effect, S_IWUSR | S_IRUGO, aw22xxx_effect_show,
		   aw22xxx_effect_store);
static DEVICE_ATTR(imax, S_IWUSR | S_IRUGO, aw22xxx_imax_show,
		   aw22xxx_imax_store);
static DEVICE_ATTR(rgb, S_IWUSR | S_IRUGO, aw22xxx_rgb_show, aw22xxx_rgb_store);
static DEVICE_ATTR(task0, S_IWUSR | S_IRUGO, aw22xxx_task0_show,
		   aw22xxx_task0_store);
static DEVICE_ATTR(task1, S_IWUSR | S_IRUGO, aw22xxx_task1_show,
		   aw22xxx_task1_store);
static DEVICE_ATTR(frq, S_IWUSR | S_IRUGO, aw22xxx_frq_show, aw22xxx_frq_store);

static struct attribute *aw22xxx_attributes[] = { &dev_attr_reg.attr,
						  &dev_attr_hwen.attr,
						  &dev_attr_fw.attr,
						  &dev_attr_cfg.attr,
...
static const struct i2c_device_id aw22xxx_i2c_id[] = { { AW22XXX_I2C_NAME, 0 },
						       {} };
