				...
				int count)
{
	...
	mutex_lock(&info->i2c_lock);
	for (i = 0; i < I2C_RETRY_CNT; ++i) {
		ret = i2c_smbus_read_i2c_block_data(info->client, addr, count,
						    data);
		if (IS_ERR_VALUE((unsigned long)ret)) {
...
{
	u8 val;
	unsigned int adj_cfg =
		cfg - 30000; /* adjust v_float bg offset (-30mV) */
...
{
	return ln8000_update_reg(info, LN8000_REG_RECOVERY_CTRL, 0x1 << 6,
				 enable << 6);
}
...
int ln8000_change_opmode(struct ln8000_info *info, unsigned int target_mode)
{
	...
	case LN8000_OPMODE_BYPASS:
		val = (0 << LN8000_BIT_STANDBY_EN) |
		      (0 << LN8000_BIT_SOFT_START_EN) |
		      (1 << LN8000_BIT_EN_1TO1);
		break;
	case LN8000_OPMODE_SWITCHING:
		val = (0 << LN8000_BIT_STANDBY_EN) |
		      (0 << LN8000_BIT_SOFT_START_EN) |
		      (0 << LN8000_BIT_EN_1TO1);
		break;
	default:
...
int psy_chg_get_ti_alarm_status(struct ln8000_info *info)
{
	...
	ln_info("st:0x%x:0x%x:0x%x:0x%x alarm=0x%x, adc_vin=%d, adc_iin=%d, adc_vbat=%d, v_offset=%d\n",
		val[0], val[1], val[2], val[3], alarm, info->vbus_uV / 1000,
		info->iin_uA / 1000, info->vbat_uV / 1000, v_offset / 1000);
...
static void ln8000_dump_regs_workfunc(struct work_struct *work)
{
	struct ln8000_info *info =
		container_of(work, struct ln8000_info, dump_regs_work.work);
