...
static int pd_get_batt_current_thermal_level(struct usbpd_pm *pdpm, int *level)
{
	...
	rc = power_supply_get_property(
		pdpm->batt_psy, POWER_SUPPLY_PROP_CHARGE_CONTROL_LIMIT, &pval);
	if (rc < 0) {
...
static bool pd_disable_cp_by_jeita_status(struct usbpd_pm *pdpm)
{
	...
	input_suspend =
		(get_client_vote(pdpm->input_suspend_votable, USER_VOTER) == 1);
	...
	rc = power_supply_get_property(pdpm->batt_psy, POWER_SUPPLY_PROP_TEMP,
				       &pval);
	if (rc < 0) {
...
static int usbpd_pm_enable_sw(struct usbpd_pm *pdpm, bool en)
{
	...
	if (!pdpm->fcc_votable || !pdpm->fv_votable || !pdpm->usb_icl_votable ||
	    !pdpm->smart_batt_votable)
		return -EINVAL;
...
static bool usbpd_get_pps_status(struct usbpd_pm *pdpm)
{
	...
	while (1) {
		...
		pr_err("cap_idx[%d], max: %d %d mv ~ %d mv, %d ma, pl: %d\n",
		       cap_idx, maxwatt, apdo_cap.min_mv, apdo_cap.max_mv,
...
static int usbpd_select_pdo(struct usbpd_pm *pdpm, enum adapter_cap_type type,
			    u32 mV, u32 mA)
{
	int ret, cnt = 0;
	...
	do {
		if (type == XM_PD_APDO_START) {
			...
			ret = tcpm_set_pd_charging_policy(
				pdpm->tcpc, DPM_CHARGING_POLICY_VSAFE5V, NULL);
...
static void usbpd_fc2_exit_work(struct work_struct *work)
{
	...
	while (pdpm->cp.vbus_volt > 6000) {
		...
		usbpd_select_pdo(pdpm, XM_PD_APDO, pdpm->request_voltage,
...
static int usbpd_pm_fc2_charge_algo(struct usbpd_pm *pdpm)
{
	...
	if (pdpm->cp.fg_vbat_mv > pm_config.bat_volt_lp_lmt - TAPER_VOL_HYS &&
		...
		if (fc2_taper_timer++ > TAPER_TIMEOUT) {
			pr_info("charge pump taper charging done, vbat[%d], ibat_curr[%d], soc[%d]\n",
				pdpm->cp.fg_vbat_mv, pdpm->cp.ibat_curr,
				capacity);
			fc2_taper_timer = 0;
	...
	if ((last_thermal_level != thermal_level) &&
	    ((thermal_level == THERMAL_LEVEL_11) ||
	     (thermal_level == THERMAL_LEVEL_12)))
		pdpm->request_voltage =
			pdpm->cp.vbat_volt * 2 + THERMAL_11_VBUS_UP;
...
static int usbpd_pm_sm(struct usbpd_pm *pdpm)
{
	...
	case PD_PM_STATE_FC2_ENTRY_1:
		...
		usbpd_select_pdo(pdpm, XM_PD_APDO_START, pdpm->request_voltage,
				 pdpm->request_current);
	...
	case PD_PM_STATE_FC2_ENTRY_2:
		...
		if (pdpm->cp.vbus_volt <
		    (pdpm->cp.vbat_volt * 2 + BUS_VOLT_INIT_UP - 100)) {
			tune_vbus_retry++;
			pdpm->request_voltage += STEP_MV;
			usbpd_select_pdo(pdpm, XM_PD_APDO,
					 pdpm->request_voltage,
					 pdpm->request_current);
	...
	case PD_PM_STATE_FC2_ENTRY_3:
		...
		if (pdpm->cp.charge_enabled) {
			if ((pm_config.cp_sec_enable &&
				...
				if (!pdpm->usb_icl_votable)
					pdpm->usb_icl_votable =
						find_votable("ICL");
				vote(pdpm->usb_icl_votable, MAIN_CHG_ICL_VOTER,
				     true, 100 * 1000);
				msleep(1000);
...
*/
	...
	ret = xm_battmngr_read_iio_prop(g_battmngr_iio, PD_PHY, PD_ACTIVE,
					&val1);
	if (ret) {
	...
	if ((pdpm->pd_active < POWER_SUPPLY_PPS_VERIFIED) &&
	    (pdpm->pd_auth_val == 1) && (val1 == POWER_SUPPLY_PD_PPS_ACTIVE)) {
		msleep(30);
		usbpd_pd_contact(pdpm, POWER_SUPPLY_PPS_VERIFIED);
		if (pdpm->fcc_votable)
			vote(pdpm->fcc_votable, NON_PPS_PD_FCC_VOTER, false, 0);
	} else if (!pdpm->pd_active && (val1 == POWER_SUPPLY_PD_PPS_ACTIVE)) {
		...
		if (!pdpm->pps_supported) {
			...
			if ((pd_cap.type[0] == 0) && (pd_cap.nr > 0)) {
				usbpd_select_pdo(pdpm, XM_PD_FIXED,
						 pd_cap.max_mv[0],
						 pd_cap.ma[0]);
				pr_err("%s: [1] cap.max_mv[%d], cap.ma[%d]\n",
				       __func__, pd_cap.max_mv[0],
				       pd_cap.ma[0]);
			}
			if ((pd_cap.type[1] == 0) && (pd_cap.nr > 1)) {
				...
				pr_err("%s: [2] cap.max_mv[%d], cap.ma[%d]\n",
				       __func__, pd_cap.max_mv[1],
				       pd_cap.ma[1]);
			}
			pr_err("%s unsupported pps, select pd fixed 2.\n",
			       __func__);
			if (pdpm->fcc_votable)
				vote(pdpm->fcc_votable, NON_PPS_PD_FCC_VOTER,
				     true, NON_PPS_PD_FCC_LIMIT);
		}
	} else if (pdpm->pd_active && !val1) {
		usbpd_pd_contact(pdpm, POWER_SUPPLY_PPS_INACTIVE);
	} else if (!pdpm->pd_active && (val1 == POWER_SUPPLY_PD_ACTIVE) &&
		   usb_present) {
		pdpm->pd_active = 1;
		pd_cap.nr = 0;
		pd_cap.selected_cap_idx = 0;
		tcpm_get_remote_power_cap(pdpm->tcpc, &pd_cap);
		if ((pd_cap.type[0] == 0) && (pd_cap.nr > 0)) {
			usbpd_select_pdo(pdpm, XM_PD_FIXED, pd_cap.max_mv[0],
					 pd_cap.ma[0]);
			pr_err("%s: [1] cap.max_mv[%d], cap.ma[%d]\n", __func__,
			       pd_cap.max_mv[0], pd_cap.ma[0]);
		}
		if ((pd_cap.type[1] == 0) && (pd_cap.nr > 1)) {
			usbpd_select_pdo(pdpm, XM_PD_FIXED, pd_cap.max_mv[1],
					 pd_cap.ma[1]);
			pr_err("%s: [2] cap.max_mv[%d], cap.ma[%d]\n", __func__,
			       pd_cap.max_mv[1], pd_cap.ma[1]);
		}
...
static int usbpd_pm_probe(struct platform_device *pdev)
{
	...
	register_tcp_dev_notifier(pdpm->tcpc, &pdpm->tcp_nb,
				  TCP_NOTIFY_TYPE_USB);
