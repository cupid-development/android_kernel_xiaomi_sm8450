...
#define stepchg_err(fmt, ...)                                                  \
	do {                                                                   \
		if (log_level >= 0)                                            \
			printk(KERN_ERR "[stepchg_jeita] " fmt,                \
			       ##__VA_ARGS__);                                 \
	} while (0)
...
static bool is_input_present(struct step_chg_info *chip)
{
	...
	if (chip->usb_psy) {
		...
		if (rc < 0)
			stepchg_err(
				"%s: Couldn't read USB Present status, rc=%d\n",
				__func__, rc);
		else
				     ...
				     int max_threshold, u32 max_value)
{
	...
	if (tuples > MAX_STEP_CHG_ENTRIES) {
		stepchg_err("%s: too many entries(%d), only %d allowed\n",
			    __func__, tuples, MAX_STEP_CHG_ENTRIES);
		return -EINVAL;
	}
	...
	for (i = 0; i < tuples; i++) {
		if (ranges[i].low_threshold > ranges[i].high_threshold) {
			stepchg_err(
				"%s: %s thresholds should be in ascendant ranges\n",
				__func__, prop_str);
			rc = -EINVAL;
			goto clean;
		}
...
static int get_step_chg_jeita_setting_from_profile(struct step_chg_info *chip)
{
	...
	rc = of_property_read_u32(node, "xm,fv-max-uv", &max_fv_uv);
	if (rc < 0) {
		pr_err("max-voltage_uv reading failed, rc=%d\n", rc);
		max_fv_uv = -EINVAL;
	}

	rc = of_property_read_u32(node, "xm,fcc-max-ua", &max_fcc_ma);
	if (rc < 0) {
	...
	if (chip->cycle_count_status == CYCLE_COUNT_NORMAL) {
		...
		if (rc < 0) {
			stepchg_err(
				"%s: Read xm,step-chg-normal-ranges failed from battery profile, rc=%d\n",
				__func__, rc);
			chip->step_chg_cfg_valid = false;
		}
	} else if (chip->cycle_count_status == CYCLE_COUNT_HIGH) {
		rc = read_range_data_from_node(node, "xm,step-chg-high-ranges",
					       chip->step_chg_config->fcc_cfg,
					       max_fv_uv, max_fcc_ma * 1000);
		if (rc < 0) {
			stepchg_err(
				"%s: Read xm,step-chg-high-ranges failed from battery profile, rc=%d\n",
				__func__, rc);
			chip->step_chg_cfg_valid = false;
		}
	}
	...
	rc = read_range_data_from_node(node, "xm,jeita-fcc-ranges",
				       chip->jeita_fcc_config->fcc_cfg,
				       BATT_HOT_DECIDEGREE_MAX,
				       max_fcc_ma * 1000);
	if (rc < 0) {
		stepchg_err(
			"%s: Read xm,jeita-fcc-ranges failed from battery profile, rc=%d\n",
			__func__, rc);
		chip->sw_jeita_cfg_valid = false;
	}

	chip->cold_step_chg_cfg_valid = true;
	rc = read_range_data_from_node(node, "xm,cold-step-chg-ranges",
				       chip->cold_step_chg_config->fcc_cfg,
				       max_fv_uv, max_fcc_ma * 1000);
	if (rc < 0) {
		stepchg_err(
			"%s: Read xm,cold-step-chg-ranges failed from battery profile, rc=%d\n",
			__func__, rc);
		chip->cold_step_chg_cfg_valid = false;
	}

	rc = read_range_data_from_node(node, "xm,jeita-fv-ranges",
				       chip->jeita_fv_config->fv_cfg,
				       BATT_HOT_DECIDEGREE_MAX, max_fv_uv);
	if (rc < 0) {
		stepchg_err(
			"%s: Read xm,jeita-fv-ranges failed from battery profile, rc=%d\n",
			__func__, rc);
		chip->sw_jeita_cfg_valid = false;
	}

	rc = of_property_read_u32_array(node, "xm,step-jeita-hysteresis",
					hysteresis, 2);
	if (!rc) {
...
static void check_cycle_count_status(struct step_chg_info *chip)
{
	...
	rc = xm_battmngr_read_iio_prop(g_battmngr_iio, BATT_FG,
				       BATT_FG_CYCLE_COUNT, &cycle_count);
	if (rc) {
...
static void get_config_work(struct work_struct *work)
{
	...
	if (rc < 0) {
		if (rc == -ENODEV || rc == -EBUSY) {
			if (chip->get_config_retry_count++ <
				...
				stepchg_info(
					"%s: bms is not ready, retry: %d\n",
					__func__, chip->get_config_retry_count);
				goto reschedule;
			}
		}
	}
	...
	for (i = 0; i < MAX_STEP_CHG_ENTRIES; i++)
		stepchg_info(
			"%s: cold-step-chg-cfg: %duV(SoC) ~ %duV(SoC), %duA\n",
			__func__,
	...
	for (i = 0; i < MAX_STEP_CHG_ENTRIES; i++)
		stepchg_info(
			"%s: jeita-fcc-cfg: %ddecidegree ~ %ddecidegre, %duA\n",
			__func__,
	...
	for (i = 0; i < MAX_STEP_CHG_ENTRIES; i++)
		stepchg_info(
			"%s: jeita-fv-cfg: %ddecidegree ~ %ddecidegre, %duV\n",
			__func__,
...
static int handle_step_chg_config(struct step_chg_info *chip)
{
	...
	rc = xm_battmngr_read_iio_prop(g_battmngr_iio, BATT_FG,
				       chip->step_chg_config->param.iio_prop,
				       &pval.intval);
	if (rc < 0) {
	...
	rc = get_val(chip->step_chg_config->fcc_cfg,
		     chip->step_chg_config->param.rise_hys,
		     chip->step_chg_config->param.fall_hys, chip->step_index,
	...
	stepchg_err(
		"%s: VBAT_NOW = %d Step-FCC = %duA step_index = %d taper-fcc: %d\n",
...
static int handle_jeita(struct step_chg_info *chip)
{
	...
	int batt_volt = 0, cold_fcc_ua = 0, volt_now = 0,
	    current_jeita_cold_fcc_index;
	...
	rc = xm_battmngr_read_iio_prop(g_battmngr_iio, BATT_FG,
				       chip->jeita_fcc_config->param.iio_prop,
				       &pval.intval);
	if (rc < 0) {
		stepchg_err("%s: Failed to read IIO prop %d rc=%d\n", __func__,
	...
	if (chip->jeita_hot_th >= 0 && chip->jeita_cold_th <= 0) {
		if ((batt_temp >= chip->jeita_hot_th ||
		     batt_temp <= chip->jeita_cold_th) &&
		    !chip->hot_cold_dis_chg) {
		...
		} else if ((batt_temp < chip->jeita_hot_th &&
			    batt_temp > chip->jeita_cold_th) &&
	...
	rc = get_val(chip->jeita_fcc_config->fcc_cfg,
		     ...
		     chip->jeita_fcc_index, pval.intval, &chip->jeita_fcc_index,
		     &fcc_ua);
	if (rc < 0)
		fcc_ua = 0;

	rc = xm_battmngr_read_iio_prop(
		g_battmngr_iio, BATT_FG,
	...
	cold_fcc_ua =
		chip->cold_step_chg_config->fcc_cfg[chip->jeita_cold_fcc_index]
			.value;
	...
	rc = get_val(chip->jeita_fv_config->fv_cfg,
		     ...
		     chip->jeita_fv_index, pval.intval, &chip->jeita_fv_index,
		     &fv_uv);
	if (rc < 0)
		fv_uv = 0;

	stepchg_err(
		"temp:%d, new jeita index:%d, new jeita FCC:%d, new jeita cold index:%d, new jeita cold FCC:%d, new jeita FV:%d\n",
		pval.intval, chip->jeita_fcc_index, fcc_ua,
		chip->jeita_cold_fcc_index, cold_fcc_ua, fv_uv);
	...
	if (g_xm_charger->pd_verified) {
		...
		if ((batt_temp >= BATT_WARM_THRESHOLD ||
		     batt_temp <= BATT_COOL_THRESHOLD) &&
		    fastcharge_mode) {
			fastcharge_mode = 0;
			xm_charger_set_fastcharge_mode(g_xm_charger,
			    ...
			    BATT_WARM_THRESHOLD - BATT_HYS_THRESHOLD) &&
			   (batt_temp >
			    BATT_COOL_THRESHOLD + BATT_HYS_THRESHOLD) &&
			   !fastcharge_mode &&
	...
	if ((batt_temp >= BATT_WARM_THRESHOLD) &&
	    (batt_volt >= BATT_WARM_VBAT_THRESHOLD) && !chip->chg_enable_flag) {
		stepchg_err(
			"sw-jeita: temp is :%d, batt_volt is :%d. stop charing\n",
			batt_temp, batt_volt);
		...
		schedule_delayed_work(
		    ...
		    (batt_volt <
		     BATT_WARM_VBAT_THRESHOLD - JEITA_SUSPEND_HYST_UV_WARM)) &&
		   chip->chg_enable_flag &&
		   (g_xm_charger->input_suspend == 0)) {
		stepchg_err(
			"sw-jeita: temp is :%d, batt_volt is :%d. start charing\n",
			batt_temp, batt_volt);
	...
	if ((batt_temp >= LIGHTING_ICON_CHANGE) && !chip->chg_change_flag) {
...
int xm_stepchg_jeita_init(struct xm_charger *charger, bool step_chg_enable,
			  bool sw_jeita_enable)
{
	...
	chip->step_chg_config = devm_kzalloc(
		chip->dev, sizeof(struct step_chg_cfg), GFP_KERNEL);
	...
	chip->cold_step_chg_config = devm_kzalloc(
		chip->dev, sizeof(struct cold_step_chg_cfg), GFP_KERNEL);
	...
	chip->jeita_fcc_config = devm_kzalloc(
		chip->dev, sizeof(struct jeita_fcc_cfg), GFP_KERNEL);
	chip->jeita_fv_config = devm_kzalloc(
		chip->dev, sizeof(struct jeita_fv_cfg), GFP_KERNEL);
