...
static ssize_t
recovery_sysfs_romboot_recovery_store(struct device *dev,
...
static int recovery_parse_romboot_ihex(void)
{
	...
	if (!(recovery_hcd->ihex_buf)) {
		LOGE(tcm_hcd->pdev->dev.parent, "No ihex data\n");
		return -ENOBUFS;
	}
	...
	for (record = 0; record < recovery_hcd->ihex_records; record++) {
		...
		retval = sscanf(&buf[record * IHEX_RECORD_SIZE],
				"%c%02x%02x%02x%02x%02x%02x", &colon, &count,
				&addrh, &addrl, &type, &data0, &data1);
		if (retval != 7) {
					  ...
					  unsigned int in_size)
{
	...
	retval = tcm_hcd->write_message(
		tcm_hcd, CMD_SPI_MASTER_WRITE_THEN_READ_EXTENDED, payld_buf,
		sizeof(flash_param) + out_size, &resp_buf, &resp_buf_size,
		&resp_length, NULL, 20);
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent,
...
static int recovery_flash_romboot_erase(struct syna_tcm_hcd *tcm_hcd)
{
	...
	LOGN(tcm_hcd->pdev->dev.parent, "%s", __func__);
	...
	retval = recovery_flash_romboot_command(tcm_hcd, JEDEC_CHIP_ERASE, NULL,
						0, NULL, 0);
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent,
					    ...
					    unsigned int image_size)
{
	...
	if ((image_size % FLASH_PAGE_SIZE) ||
		...
		LOGE(tcm_hcd->pdev->dev.parent, "Wrong image file");
		LOGE(tcm_hcd->pdev->dev.parent,
		     "image_size = %d, img_header = 0x%04x", image_size,
		     img_header);
		return -EINVAL;
	}
	...
	LOGE(tcm_hcd->pdev->dev.parent,
	     "image_size = %d, img_header = 0x%04x, pages = %d", image_size,
	     img_header, pages);

	for (idx = 0; idx < pages; idx++) {
		retval = recovery_flash_romboot_command(
			tcm_hcd, JEDEC_WRITE_ENABLE, NULL, 0, NULL, 0);
		if (retval < 0) {
			LOGE(tcm_hcd->pdev->dev.parent,
			     "Failed to write JEDEC_WRITE_ENABLE, retval=%d\n",
			     retval);
		...
		retval = recovery_flash_romboot_command(
			tcm_hcd, JEDEC_PAGE_PROGRAM, buf, sizeof(buf), NULL, 0);
		if (retval < 0) {
			LOGE(tcm_hcd->pdev->dev.parent,
			     "Failed to write JEDEC_READ_STATUS, retval=%d\n",
			     retval);
...
static void recovery_do_romboot_recovery(struct syna_tcm_hcd *tcm_hcd)
{
	...
	LOGN(tcm_hcd->pdev->dev.parent, "%s\n", __func__);
	...
	if (tcm_hcd->id_info.mode != MODE_ROMBOOTLOADER) {
		LOGE(tcm_hcd->pdev->dev.parent, "Not in romboot\n");
	...
	LOGN(tcm_hcd->pdev->dev.parent, "image_size = %d\n", image_size);
	...
	LOGN(tcm_hcd->pdev->dev.parent, "Do recovery_flash_romboot_reprogram");
	...
	LOGN(tcm_hcd->pdev->dev.parent, "Do reset and re-init");
	retval = tcm_hcd->reset_n_reinit(tcm_hcd, true, true);
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent, "Failed to reset, retval=%d\n",
		     retval);
	}
						 ...
						 const char *buf, size_t count)
{
	...
	if (recovery_hcd->ihex_size == 0) {
		LOGE(tcm_hcd->pdev->dev.parent, "Failed to get ihex data\n");
		retval = -EINVAL;
		goto exit;
	}

	if (recovery_hcd->ihex_size % IHEX_RECORD_SIZE) {
		LOGE(tcm_hcd->pdev->dev.parent, "Invalid ihex data\n");
		retval = -EINVAL;
		goto exit;
	}
					 ...
					 char *buf, loff_t pos, size_t count)
{
	...
	retval = secure_memcpy(&recovery_hcd->ihex_buf[pos],
			       IHEX_BUF_SIZE - pos, buf, count, count);
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent,
...
static int recovery_device_reset(void)
{
	...
	retval = syna_tcm_rmi_write(
		tcm_hcd, recovery_hcd->f35_addr.control_base + F35_CTRL3_OFFSET,
...
static int recovery_check_status(void)
{
	...
	retval = syna_tcm_rmi_read(tcm_hcd, recovery_hcd->f35_addr.data_base,
...
static int recovery_write_flash(void)
{
	...
	while (entries_written < recovery_hcd->data_entries) {
		...
		retval = secure_memcpy(
			recovery_hcd->chunk_buf, chunk_buf_size - 1, data_ptr,
...
static int recovery_poll_erase_completion(void)
{
	...
	do {
		...
		do {
			retval = syna_tcm_rmi_read(
				tcm_hcd, recovery_hcd->f35_addr.command_base,
				&command, sizeof(command));
			if (retval < 0) {
				LOGE(tcm_hcd->pdev->dev.parent,
		...
		retval =
			syna_tcm_rmi_read(tcm_hcd, data_base + F35_DATA5_OFFSET,
					  &status, sizeof(status));
		if (retval < 0) {
			LOGE(tcm_hcd->pdev->dev.parent,
...
static int recovery_set_up_recovery_mode(void)
{
	...
	retval = tcm_hcd->write_message(tcm_hcd, recovery_hcd->out_buf[0],
					&recovery_hcd->out_buf[1], 2, NULL,
					NULL, NULL, NULL, 0);
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent,
...
static int recovery_do_f35_recovery(void)
{
	...
	if (p_entry.fn_number != UBL_FN_NUMBER) {
		LOGE(tcm_hcd->pdev->dev.parent, "Failed to find F$35\n");
		return -ENODEV;
	}
	...
	LOGN(tcm_hcd->pdev->dev.parent, "Start of recovery\n");

	retval = recovery_erase_flash();
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent, "Failed to erase flash\n");
		return retval;
	}

	LOGN(tcm_hcd->pdev->dev.parent, "Flash erased\n");
	...
	LOGN(tcm_hcd->pdev->dev.parent, "Flash written\n");
	...
	LOGN(tcm_hcd->pdev->dev.parent, "End of recovery\n");
