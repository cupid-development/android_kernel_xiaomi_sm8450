...
module_param_named(tune_debug_enable, tune_debug_enable, int,
...
static void syna_print_xfer_data(struct syna_tcm_hcd *tcm_hcd,
				 ...
				 unsigned int wr_rd)
{
	...
	if ((wr_rd == SYNA_SPI_TRANSFER_READ) &&
	    (data[0] == SYNA_TCM_MESSAGE_MARKER)) {
		code = data[1];
	}
	...
	if (tune_debug_enable == 1) {
		...
		if (((code == STATUS_CONTINUED_READ) &&
		     (last_code > REPORT_IDENTIFY)) ||
		    (code > REPORT_IDENTIFY))
			goto exit;
	} else if (tune_debug_enable == 3) {
		/* skip the report (> REPORT_TOUCH) */
		if (((code == STATUS_CONTINUED_READ) &&
		     (last_code > REPORT_TOUCH)) ||
		    (code > REPORT_TOUCH))
			goto exit;
	} else {
		goto exit;
	...
	for (i = 0; i < length; i++) {
		...
		cnt = snprintf(print_buf + offset,
			       SYNA_SPI_PRINT_BUF_SIZE - offset, "%02x ",
			       data[i]);
		offset += cnt;
	}
	cnt = snprintf(print_buf + offset, SYNA_SPI_PRINT_BUF_SIZE - offset,
...
static int parse_dt(struct device *dev, struct syna_tcm_board_data *bdata)
{
	...
	if (prop && prop->length) {
		bdata->irq_gpio = of_get_named_gpio_flags(
			np, "synaptics,irq-gpio", 0,
	...
	if (prop && prop->length) {
		bdata->power_gpio = of_get_named_gpio_flags(
			np, "synaptics,power-gpio", 0, NULL);
	} else {
		bdata->power_gpio = -1;
	}
	...
	if (!retval) {
		...
		if (strlen(name_tmp) < sizeof(bdata->avdd_name))
			strncpy(bdata->avdd_name, name_tmp,
				sizeof(bdata->avdd_name));
		else
			LOGE(dev, "invalied avdd name length: %ld > %ld",
			     strlen(name_tmp), sizeof(bdata->avdd_name));
	}
	...
	if (!retval) {
		...
		if (strlen(name_tmp) < sizeof(bdata->iovdd_name))
			strncpy(bdata->iovdd_name, name_tmp,
				sizeof(bdata->iovdd_name));
		else
			LOGE(dev, "invalied iovdd name length: %ld > %ld",
			     strlen(name_tmp), sizeof(bdata->iovdd_name));
	}

	prop = of_find_property(np, "synaptics,reset-gpio", NULL);
	if (prop && prop->length) {
		bdata->reset_gpio = of_get_named_gpio_flags(
			np, "synaptics,reset-gpio", 0, NULL);
	} else {
		bdata->reset_gpio = -1;
	}
				  ...
				  unsigned int count, unsigned int size)
{
	...
	if (count > xfer_count) {
		...
		if (!xfer) {
			LOGE(&spi->dev, "Failed to allocate memory for xfer\n");
			xfer_count = 0;
			return -ENOMEM;
		}
		xfer_count = count;
	...
	if (size > buf_size) {
		...
		if (!buf) {
			LOGE(&spi->dev, "Failed to allocate memory for buf\n");
			buf_size = 0;
			return -ENOMEM;
		}
		buf_size = size;
				 ...
				 unsigned int length)
{
	...
	if (retval < 0) {
		LOGE(&spi->dev, "Failed to allocate memory, retval=%d\n",
		     retval);
		goto exit;
	}
	...
	if (retval == 0) {
		...
		LOGE(&spi->dev, "Failed to complete SPI transfer, error = %d\n",
		     retval);
	}
				  ...
				  unsigned int length)
{
	...
	if (retval < 0) {
		LOGE(&spi->dev, "Failed to copy write data, retval=%d\n",
		     retval);
		goto exit;
	}
			     ...
			     unsigned int length)
{
	...
	if (tcm_hcd->tp_pm_suspend) {
		...
		retval = wait_for_completion_timeout(
			&tcm_hcd->pm_resume_completion, msecs_to_jiffies(500));
		if (!retval) {
	...
	if (retval == 0) {
		retval = length;
		syna_print_xfer_data(tcm_hcd, data, length,
				     SYNA_SPI_TRANSFER_READ);
	} else {
		LOGE(&spi->dev, "Failed to complete SPI transfer, error = %d\n",
		     retval);
	}
...
static int syna_tcm_spi_probe(struct spi_device *spi)
{
	...
	if (spi->master->flags & SPI_MASTER_HALF_DUPLEX) {
		LOGE(&spi->dev, "Full duplex not supported by host\n");
		return -EIO;
	}

	syna_tcm_spi_device = platform_device_alloc(PLATFORM_DRIVER_NAME, 0);
	if (!syna_tcm_spi_device) {
		LOGE(&spi->dev, "Failed to allocate platform device\n");
		return -ENOMEM;
	}
	...
	if (!hw_if.bdata) {
		LOGE(&spi->dev, "Failed to allocate memory for board data\n");
		return -ENOMEM;
	}
	...
	if (retval < 0) {
		LOGE(&spi->dev, "Failed to add platform device, retval=%d\n",
		     retval);
		return retval;
	}
