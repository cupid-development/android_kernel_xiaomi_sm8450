...
static void syna_tcm_early_suspend_work(struct work_struct *work)
{
	...
	if (!tcm_hcd->in_sleep)
		...
		LOGN(tcm_hcd->pdev->dev.parent,
		     "tp fw is in sleep, skip palm area change setting\n");
...
static void syna_tcm_set_report_rate_work(struct work_struct *work)
{
	...
	/* set dynamic config */
	val = tcm_hcd->report_rate_mode &
	      0x03; /* 0:default(180Hz), 1:240Hz 2: 180Hz, 3:120Hz */
...
static void syna_tcm_fod_work(struct work_struct *work)
{
	...
	LOGI(tcm_hcd->pdev->dev.parent,
	     "set touch and hold enable control, value = 0x%02x\n", val);
	retval = tcm_hcd->set_dynamic_config(tcm_hcd, DC_ENABLE_TOUCH_AND_HOLD,
					     val);
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent,
...
static void syna_tcm_deadzone_rejection(bool on, int direction,
					struct syna_grip_zone *zone)
{
	int i = 0;
	struct syna_tcm_hcd *tcm_hcd = gloab_tcm_hcd;
	const struct syna_tcm_xiaomi_board_data *bdata =
		&tcm_hcd->xiaomi_board_data;
#ifdef GRIP_MODE_DEBUG
	int *pVal;

	pVal = (int *)&(bdata->deadzone_filter_hor[0]);
	LOGI(tcm_hcd->pdev->dev.parent,
	     "deadzone hor %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d\n",
	     pVal[0], pVal[1], pVal[2], pVal[3], pVal[4], pVal[5], pVal[6],
	     pVal[7], pVal[8], pVal[9], pVal[10], pVal[11], pVal[12], pVal[13],
	     pVal[14], pVal[15], pVal[16], pVal[17], pVal[18], pVal[19],
	     pVal[20], pVal[21], pVal[22], pVal[23], pVal[24], pVal[25],
	     pVal[26], pVal[27], pVal[28], pVal[29], pVal[30], pVal[31]);
	pVal = (int *)&(bdata->deadzone_filter_ver[0]);
	LOGI(tcm_hcd->pdev->dev.parent,
	     "deadzone ver %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d\n",
	     pVal[0], pVal[1], pVal[2], pVal[3], pVal[4], pVal[5], pVal[6],
	     pVal[7], pVal[8], pVal[9], pVal[10], pVal[11], pVal[12], pVal[13],
	     pVal[14], pVal[15], pVal[16], pVal[17], pVal[18], pVal[19],
	     pVal[20], pVal[21], pVal[22], pVal[23], pVal[24], pVal[25],
	...
	if (direction) {
		for (i = 0; i < GRIP_RECT_NUM * GRIP_PARAMETER_NUM / 3;
			...
			syna_tcm_set_grip_rect(
				(int *)&(bdata->deadzone_filter_hor[i]), zone);
		}
	} else {
		for (i = 0; i < GRIP_RECT_NUM * GRIP_PARAMETER_NUM / 3;
		     i += GRIP_PARAMETER_NUM) {
			syna_tcm_set_grip_rect(
				(int *)&(bdata->deadzone_filter_ver[i]), zone);
		}
	}
...
static void syna_tcm_edge_rejection(bool on, int direction,
				    struct syna_grip_zone *zone)
{
	int i = 0;
	struct syna_tcm_hcd *tcm_hcd = gloab_tcm_hcd;
	...
	LOGI(tcm_hcd->pdev->dev.parent,
	     "Edge hor %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d\n",
	     pVal[0], pVal[1], pVal[2], pVal[3], pVal[4], pVal[5], pVal[6],
	     pVal[7], pVal[8], pVal[9], pVal[10], pVal[11], pVal[12], pVal[13],
	     pVal[14], pVal[15], pVal[16], pVal[17], pVal[18], pVal[19],
	     pVal[20], pVal[21], pVal[22], pVal[23], pVal[24], pVal[25],
	...
	LOGI(tcm_hcd->pdev->dev.parent,
	     "Edge ver %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d\n",
	     pVal[0], pVal[1], pVal[2], pVal[3], pVal[4], pVal[5], pVal[6],
	     pVal[7], pVal[8], pVal[9], pVal[10], pVal[11], pVal[12], pVal[13],
	     pVal[14], pVal[15], pVal[16], pVal[17], pVal[18], pVal[19],
	     pVal[20], pVal[21], pVal[22], pVal[23], pVal[24], pVal[25],
	...
	if (direction) {
		for (i = 0; i < GRIP_RECT_NUM * GRIP_PARAMETER_NUM / 3;
		     i += GRIP_PARAMETER_NUM) {
			syna_tcm_set_grip_rect(
				(int *)&(bdata->edgezone_filter_hor[i]), zone);
		}
	} else {
		for (i = 0; i < GRIP_RECT_NUM * GRIP_PARAMETER_NUM / 3;
		     i += GRIP_PARAMETER_NUM) {
			syna_tcm_set_grip_rect(
				(int *)&(bdata->edgezone_filter_ver[i]), zone);
		}
	}
...
static void syna_tcm_corner_rejection(bool on, int direction,
				      struct syna_grip_zone *zone)
{
	struct syna_tcm_hcd *tcm_hcd = gloab_tcm_hcd;
	...
	LOGI(tcm_hcd->pdev->dev.parent,
	     "Corner hor1 %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d\n",
	     pVal[0], pVal[1], pVal[2], pVal[3], pVal[4], pVal[5], pVal[6],
	     pVal[7], pVal[8], pVal[9], pVal[10], pVal[11], pVal[12], pVal[13],
	     pVal[14], pVal[15], pVal[16], pVal[17], pVal[18], pVal[19],
	     pVal[20], pVal[21], pVal[22], pVal[23], pVal[24], pVal[25],
	...
	LOGI(tcm_hcd->pdev->dev.parent,
	     "cornerzone_ver %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d\n",
	     pVal[0], pVal[1], pVal[2], pVal[3], pVal[4], pVal[5], pVal[6],
	     pVal[7], pVal[8], pVal[9], pVal[10], pVal[11], pVal[12], pVal[13],
	     pVal[14], pVal[15], pVal[16], pVal[17], pVal[18], pVal[19],
	     pVal[20], pVal[21], pVal[22], pVal[23], pVal[24], pVal[25],
	...
	switch (p_xiaomi_touch_interfaces
			->touch_mode[Touch_Edge_Filter][SET_CUR_VALUE]) {
	case 0:
		filter_value = 0;
		break;
	...
	default:
		filter_value = bdata->cornerfilter_area_step2;
		LOGI(tcm_hcd->pdev->dev.parent,
		     "%s: no support value use default filter x/y value\n",
		     __func__);
		break;
	}
	...
	if (direction == 1) {
		...
		pVal = (int *)&(bdata->cornerzone_filter_hor1[0]);
		LOGI(tcm_hcd->pdev->dev.parent,
		     "direction == 1 cornerzone_hor1 %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d\n",
		     pVal[0], pVal[1], pVal[2], pVal[3], pVal[4], pVal[5],
		     pVal[6], pVal[7], pVal[8], pVal[9], pVal[10], pVal[11],
		     pVal[12], pVal[13], pVal[14], pVal[15], pVal[16], pVal[17],
		     pVal[18], pVal[19], pVal[20], pVal[21], pVal[22], pVal[23],
		     pVal[24], pVal[25], pVal[26], pVal[27], pVal[28], pVal[29],
		     pVal[30], pVal[31]);
#endif
	} else if (direction == 3) {
		...
		pVal = (int *)&(bdata->cornerzone_filter_hor2[0]);
		LOGI(tcm_hcd->pdev->dev.parent,
		     "direction == 3 cornerzone_hor2 %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d\n",
		     pVal[0], pVal[1], pVal[2], pVal[3], pVal[4], pVal[5],
		     pVal[6], pVal[7], pVal[8], pVal[9], pVal[10], pVal[11],
		     pVal[12], pVal[13], pVal[14], pVal[15], pVal[16], pVal[17],
		     pVal[18], pVal[19], pVal[20], pVal[21], pVal[22], pVal[23],
		...
		pVal = (int *)&(bdata->cornerzone_filter_ver[0]);
		LOGI(tcm_hcd->pdev->dev.parent,
		     "direction == 0 cornerzone_ver %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d\n",
		     pVal[0], pVal[1], pVal[2], pVal[3], pVal[4], pVal[5],
		     pVal[6], pVal[7], pVal[8], pVal[9], pVal[10], pVal[11],
		     pVal[12], pVal[13], pVal[14], pVal[15], pVal[16], pVal[17],
		     pVal[18], pVal[19], pVal[20], pVal[21], pVal[22], pVal[23],
...
{
	bool gamemode_on = p_xiaomi_touch_interfaces
	...
	*/
	...
	if (!gamemode_on) {
		...
		for (i = 0; i < GRIP_RECT_NUM * GRIP_PARAMETER_NUM;
		     i += GRIP_PARAMETER_NUM) {
			syna_tcm_set_grip_rect(
				&(p_xiaomi_touch_interfaces->long_mode_value[i]),
				grip_zone);
		}
...
send_cmd:
	...
	out_buf[length_out - 1] =
		(unsigned char)((checksum & 0xFF) + ((checksum >> 8) & 0xFF));
	retval = tcm_hcd->write_message(tcm_hcd, CMD_MultiFunction, out_buf,
					length_out, &resp_buf, &resp_buf_size,
					&resp_length, NULL, 0);
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent,
		     "Failed to write command %s, retval=%d\n",
	...
	offset = 0;
	LOGI(tcm_hcd->pdev->dev.parent,
	     "length_out:%d, SUBID %d, gamemode|panel_orientation: %02x %02x, crc8:%d checksum: %d\n",
	     length_out, out_buf[0], out_buf[1], out_buf[2],
	     out_buf[length_out - 1], checksum);
	//offset = 3;
	for (i = 0; i < 3 * 4; i++) {
		offset = 3 + i * 8;
#ifdef GRIP_MODE_DEBUG
		LOGI(tcm_hcd->pdev->dev.parent,
		     "%s corner[%d]:%4d %4d %4d %4d\n", __func__, i % 4,
		     le2_to_uint(&out_buf[offset + 0]),
		     le2_to_uint(&out_buf[offset + 2]),
		     le2_to_uint(&out_buf[offset + 4]),
		     le2_to_uint(&out_buf[offset + 6]));
#endif
	}
...
static void syna_tcm_get_grip_mode(unsigned int *gamemode,
				   unsigned int *panel_orientation)
{
	int retval;
	int i;
	unsigned char out_buf;
	...
	*panel_orientation = resp_buf[0] & 0x0F;
	...
	offset = 0;
	LOGI(tcm_hcd->pdev->dev.parent,
	     "length:%d, gamemode|panel_orientation: %d %d, checksum: %d\n",
	...
	for (i = 0; i < 3 * 4; i++) {
		...
		LOGI(tcm_hcd->pdev->dev.parent,
		     "%s, corner[%d]:%4d %4d %4d %4d\n", __func__, i % 4,
		     le2_to_uint(&resp_buf[offset + 0]),
		     le2_to_uint(&resp_buf[offset + 2]),
		     le2_to_uint(&resp_buf[offset + 4]),
		     le2_to_uint(&resp_buf[offset + 6]));
#endif
	}
...
static void syna_tcm_update_touchmode_data(void)
{
	...
	for (i = 0; i < Touch_Mode_NUM; i++) {
		if (p_xiaomi_touch_interfaces->touch_mode[i][GET_CUR_VALUE] !=
			...
			temp_value = p_xiaomi_touch_interfaces
					     ->touch_mode[i][SET_CUR_VALUE];
			...
			retval = tcm_hcd->set_dynamic_config(tcm_hcd, dc_id,
							     temp_value);
			if (retval < 0) {
				LOGE(tcm_hcd->pdev->dev.parent,
				     "set dynamic config id:0x%02x failed, retval=%d\n",
				     dc_id, retval);
			}

			// debug: read to get the current value in FW
			retval = tcm_hcd->get_dynamic_config(tcm_hcd, dc_id,
							     &temp_value);
			if (retval < 0) {
				LOGE(tcm_hcd->pdev->dev.parent,
				     "Failed to get dynamic config id: 0x%02x, retval=%d\n",
				     dc_id, retval);
			} else {
				LOGI(tcm_hcd->pdev->dev.parent,
				     "get dynamic config id: 0x%02x, value: %d\n",
				     dc_id, temp_value);
			}
		}
	}
	...
	if ((mode_update_flag & (1 << Touch_Game_Mode)) ||
	    (tcm_hcd->gamemode_enable &&
	     (mode_update_flag & (1 << Touch_Panel_Orientation))) ||
	    (tcm_hcd->gamemode_enable &&
	     (mode_update_flag & (1 << Touch_Edge_Filter)))) {
		LOGI(tcm_hcd->pdev->dev.parent,
		     "gamemode_enabled: %d, mode_update_flag: %d\n",
...
static void syna_tcm_grip_mode_work(struct work_struct *work)
{
	...
	if (long_mode_len != GRIP_RECT_NUM * GRIP_PARAMETER_NUM) {
		LOGE(tcm_hcd->pdev->dev.parent, "%s len is invalid\n",
		     __func__);
		return;
	}
...
static int syna_tcm_read_touchmode_data(void)
{
	...
	LOGI(tcm_hcd->pdev->dev.parent,
	     ...
	     get_value[Touch_Game_Mode], get_value[Touch_Active_MODE],
	     get_value[Touch_UP_THRESHOLD], get_value[Touch_Tolerance],
	     get_value[Touch_Panel_Orientation], get_value[Touch_Report_Rate]);
...
static int syna_tcm_palm_area_change_setting(int value)
{
	...
	LOGI(tcm_hcd->pdev->dev.parent, "palm area change setting, 0x%02x\n",
	     value);

	retval = tcm_hcd->set_dynamic_config(tcm_hcd, DC_PALM_AREA_CHANGE,
					     value);
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent,
...
int syna_tcm_palm_sensor_write(int value)
{
	...
	LOGI(tcm_hcd->pdev->dev.parent, "palm_sensor_write, 0x%02x\n", value);
...
static int syna_tcm_set_cur_value(int mode, int val)
{
	...
	case Touch_Fod_Enable:
		...
		if (val == FOD_STATUS_INVALID || val == FOD_STATUS_DELETED) {
		...
		} else if (val == FOD_STATUS_DISABLED ||
			   val == FOD_STATUS_INPUT_FINGERPRINT) {
			...
			if (tcm_hcd->in_sleep && tcm_hcd->in_suspend &&
			    tcm_hcd->flag_sleep) {
				LOGI(tcm_hcd->pdev->dev.parent,
				     "Touch_Fod_Enable value:FOD_STATUS_AUTHENTICATION, Exit sleep mode!\n");
				/* enable irq */
				tcm_hcd->enable_irq(tcm_hcd, true, NULL);
				retval = tcm_hcd->sleep(tcm_hcd, false);
				if (retval < 0) {
		...
		LOGI(tcm_hcd->pdev->dev.parent,
		     "Touch_Fod_Enable set fod_enabled to %d , set finger_unlock_status to %d\n",
	...
	case Touch_Panel_Orientation:
		LOGI(tcm_hcd->pdev->dev.parent,
		     "Touch_Panel_Orientation value [%d]\n", val);
		break;
	case Touch_Edge_Filter:
		LOGI(tcm_hcd->pdev->dev.parent,
		     "Touch_Edge_Filter value [%d]\n", val);
		break;
	case Touch_UP_THRESHOLD:
		LOGI(tcm_hcd->pdev->dev.parent,
		     "Touch_UP_THRESHOLD value [%d]\n", val);
		break;
	...
	default:
		LOGI(tcm_hcd->pdev->dev.parent,
		     "other mode type mode [%d], value [%d]\n", mode, val);
		break;
	}
...
static int syna_tcm_set_mode_long_value(int mode, int len, int *buf)
{
	...
	for (i = 0; i < len; i = i + 8) {
		LOGI(tcm_hcd->pdev->dev.parent,
		     ...
		     p_xiaomi_touch_interfaces->long_mode_value[i],
		     p_xiaomi_touch_interfaces->long_mode_value[i + 1],
		     p_xiaomi_touch_interfaces->long_mode_value[i + 2],
		     p_xiaomi_touch_interfaces->long_mode_value[i + 3],
		     p_xiaomi_touch_interfaces->long_mode_value[i + 4],
		     p_xiaomi_touch_interfaces->long_mode_value[i + 5],
		     p_xiaomi_touch_interfaces->long_mode_value[i + 6],
		     p_xiaomi_touch_interfaces->long_mode_value[i + 7]);
	}
#endif
	mutex_unlock(&tcm_hcd->long_mode_value_mutex);
...
static int syna_tcm_get_mode_all(int mode, int *val)
{
	...
	if ((mode < Touch_Mode_NUM) && (mode >= 0)) {
		val[0] = p_xiaomi_touch_interfaces
				 ->touch_mode[mode][GET_CUR_VALUE];
		val[1] = p_xiaomi_touch_interfaces
				 ->touch_mode[mode][GET_DEF_VALUE];
		val[2] = p_xiaomi_touch_interfaces
				 ->touch_mode[mode][GET_MIN_VALUE];
		val[3] = p_xiaomi_touch_interfaces
...
static int
syna_tcm_parse_gamemode_param_dt(struct syna_tcm_xiaomi_board_data *bdata)
{
	int retval, i, j;
	...
	for (i = 0; i < Touch_Mode_NUM; i++) {
		...
		if (prop && prop->length) {
			retval = of_property_read_u32_array(
				np, str_dts, temp_val,
				SYNA_TOUCH_MODE_PARAMETERS_SIZE);
			if (retval < 0) {
				LOGE(tcm_hcd->pdev->dev.parent,
				     "Failed to read synaptics, %s property, retval=%d\n",
				     str_dts, retval);
			}
		}
	...
	if (prop && prop->length) {
		retval = of_property_read_u32_array(
			np, str_dts, temp_val,
			SYNA_CORNERFILTER_AREA_STEP_SIZE);
		if (retval < 0) {
			LOGE(tcm_hcd->pdev->dev.parent,
			     "Failed to read synaptics, %s property, retval=%d\n",
			     str_dts, retval);
...
#ifdef GRIP_MODE_DEBUG
		LOGI(tcm_hcd->pdev->dev.parent,
		     "%s: %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d\n",
		     str_dts, pVal[0], pVal[1], pVal[2], pVal[3], pVal[4],
		     pVal[5], pVal[6], pVal[7], pVal[8], pVal[9], pVal[10],
		     pVal[11], pVal[12], pVal[13], pVal[14], pVal[15], pVal[16],
		     pVal[17], pVal[18], pVal[19], pVal[20], pVal[21], pVal[22],
		     pVal[23], pVal[24], pVal[25], pVal[26], pVal[27], pVal[28],
		     pVal[29], pVal[30], pVal[31]);
#endif
	}
	...
	if (prop && prop->length) {
		retval = of_property_read_u32_array(
			np, str_dts, temp_val, SYNA_DISPLAY_RESOLUTION_SIZE);
		if (retval < 0) {
			LOGE(tcm_hcd->pdev->dev.parent,
			     "Failed to read synaptics, %s property, retval=%d\n",
			     str_dts, retval);
...
static void syna_tcm_init_touchmode_data(void)
{
	...
	/* Finger Hysteresis */
	p_xiaomi_touch_interfaces
		->touch_mode[Touch_UP_THRESHOLD][GET_MAX_VALUE] =
		bdata->up_threshold[SYNA_DTS_GET_MAX_INDEX];
	p_xiaomi_touch_interfaces
		->touch_mode[Touch_UP_THRESHOLD][GET_MIN_VALUE] =
		bdata->up_threshold[SYNA_DTS_GET_MIN_INDEX];
	p_xiaomi_touch_interfaces
		->touch_mode[Touch_UP_THRESHOLD][GET_DEF_VALUE] =
	...
	p_xiaomi_touch_interfaces
		->touch_mode[Touch_UP_THRESHOLD][GET_CUR_VALUE] =
	...
	/* Orientation */
	p_xiaomi_touch_interfaces
		->touch_mode[Touch_Panel_Orientation][GET_MAX_VALUE] =
		bdata->panel_orien[SYNA_DTS_GET_MAX_INDEX];
	p_xiaomi_touch_interfaces
		->touch_mode[Touch_Panel_Orientation][GET_MIN_VALUE] =
	...
	for (i = 0; i < Touch_Mode_NUM; i++) {
		LOGI(tcm_hcd->pdev->dev.parent,
		     ...
		     i, p_xiaomi_touch_interfaces->touch_mode[i][SET_CUR_VALUE],
		     p_xiaomi_touch_interfaces->touch_mode[i][GET_CUR_VALUE],
				      ...
				      size_t count, loff_t *pos)
{
	...
	LOGD(tcm_hcd->pdev->dev.parent,
	     "Before: transfer_size = %d, left_size = %d, pos = %d\n",
	...
	if (*pos == 0) {
		...
		if (!tcm_hcd->testing_xiaomi_report_data) {
			LOGE(tcm_hcd->pdev->dev.parent,
			     "tcm_hcd->testing_xiaomi_report_data = NULL\n");
			ret = -EINVAL;
			goto err_out;
		}

		cnt1 = tcm_hcd->testing_xiaomi_report_data(
			REPORT_DELTA, tmp); /* REPORT_DELTA = 0x12 */
		//cnt1 = testing_xiaomi_report_data(REPORT_DELTA, tmp); /* REPORT_DELTA = 0x12 */
		LOGD(tcm_hcd->pdev->dev.parent, "cnt1 = %d\n", cnt1);
		if (cnt1 <= 0) {
			LOGE(tcm_hcd->pdev->dev.parent,
			     "REPORT_DELTA failed, cnt1=%d\n", cnt1);
			ret = cnt1;
			goto err_out;
		}

		cnt2 = tcm_hcd->testing_xiaomi_report_data(
			REPORT_RID161, tmp + cnt1); /* REPORT_RID161 = 0xA1 */
		//cnt2 = testing_xiaomi_report_data(REPORT_RID161, tmp + cnt1); /* REPORT_RID161 = 0xA1 */
		LOGD(tcm_hcd->pdev->dev.parent, "cnt2 = %d\n", cnt2);
		if (cnt2 <= 0) {
			LOGE(tcm_hcd->pdev->dev.parent,
			     "REPORT_RID161 failed, cnt2=%d\n", cnt2);
			ret = cnt2;
			goto err_out;
		}
		sum_size = cnt1 + cnt2;
	...
	LOGD(tcm_hcd->pdev->dev.parent,
	     "1: sum_size = %d, left_size = %d, transfer_size = %d\n", sum_size,
	     left_size, transfer_size);
	...
	if (access_ok(buf, sum_size)) {
		...
		if (ret != 0) {
			LOGE(tcm_hcd->pdev->dev.parent,
			     "copy to user failed, ret = %d\n", ret);
			ret = -EFAULT;
			goto err_out;
		} else {
			LOGD(tcm_hcd->pdev->dev.parent,
			     "copy to user success, ret = %d\n", ret);
		}
	}

	*pos += transfer_size;
	LOGD(tcm_hcd->pdev->dev.parent,
	     "After: transfer_size = %d, left_size = %d, pos = %d\n",
	     transfer_size, left_size, *pos);
...
static ssize_t syna_tcm_selftest_write(struct file *file,
				       ...
				       loff_t *pos)
{
	...
	if (!strncmp("short", tmp, 5) || !strncmp("open", tmp, 4)) {
		...
		wait_event_interruptible(tcm_hcd->selftest_wait_queue,
					 tcm_hcd->selftest_ready);
...
*/
...
#ifdef SYNA_TCM_XIAOMI_TOUCHFEATURE
	tcm_hcd->game_wq =
		alloc_workqueue("syna_tcm_game_queue",
				WQ_UNBOUND | WQ_HIGHPRI | WQ_CPU_INTENSIVE, 1);
	if (!tcm_hcd->game_wq) {
		LOGE(tcm_hcd->pdev->dev.parent, "Failed to create game_wq\n");
		retval = -ENOMEM;
	...
	if (!p_xiaomi_touch_interfaces) {
		p_xiaomi_touch_interfaces = kmalloc(
	...
	memset(p_xiaomi_touch_interfaces, 0x00,
	       sizeof(struct xiaomi_touch_interface));
	p_xiaomi_touch_interfaces->setModeValue = syna_tcm_set_cur_value;
	p_xiaomi_touch_interfaces->setModeLongValue =
		syna_tcm_set_mode_long_value;
	...
	p_xiaomi_touch_interfaces->palm_sensor_write =
		syna_tcm_palm_sensor_write;
	...
	p_xiaomi_touch_interfaces->panel_vendor_read =
		syna_tcm_panel_vendor_read;
	p_xiaomi_touch_interfaces->panel_color_read = syna_tcm_panel_color_read;
	p_xiaomi_touch_interfaces->panel_display_read =
		syna_tcm_panel_display_read;
	p_xiaomi_touch_interfaces->touch_vendor_read =
		syna_tcm_touch_vendor_read;
	...
	tcm_hcd->tp_lockdown_info_proc = proc_create(
		"tp_lockdown_info", 0444, NULL, &syna_tcm_lockdown_info_ops);
	tcm_hcd->tp_fw_version_proc = proc_create("tp_fw_version", 0444, NULL,
						  &syna_tcm_fw_version_ops);
