...
int parseBinFile(u8 *fw_data, int fw_size, Firmware *fwData, int keep_cx)
{
	...
	if (fw_size < FW_HEADER_SIZE + FW_BYTES_ALLIGN || fw_data == NULL) {
		...
		logError(1,
			 "%s Code Pages: %d panel area Pages: %d"
			 " cx area Pages: %d fw config Pages: %d !\n",
			 tag, fwData->fw_code_size, fwData->panel_config_size,
		...
		logError(1,
			 "%s Code start addr: 0x%08X cx start addr: 0x%08X"
			 " fw start addr: 0x%08X !\n",
			 tag, fwData->code_start_addr, fwData->cx_start_addr,
			 fwData->config_start_addr);
...
int flash_erase_page_by_page(ErasePage keep_cx, Firmware *fw)
{
	...
	u8 cmd[6] = { FTS_CMD_HW_REG_W,	 0x20, 0x00, 0x00,
		      FLASH_ERASE_CODE0, 0x80 };

	u8 cmd2[11] = { FTS_CMD_HW_REG_W,
			0x20,
			0x00,
			0x01,
			0x28,
	...
	if ((fw->fw_code_size == 0) || (fw->panel_config_size == 0) ||
		...
		if (fw->cx_area_size > 1)
			flash_cx_end_page =
				flash_cx_start_page + (fw->cx_area_size - 1);
		else
	...
	logError(
		1,
		"%s CX Start page: %d CX End page: %d Panel Start Page: %d Panel End page: %d!\n",
		tag, flash_cx_start_page, flash_cx_end_page,
		flash_panel_start_page, flash_panel_end_page);

	for (i = flash_cx_start_page;
	     i <= flash_cx_end_page && keep_cx >= SKIP_PANEL_CX_INIT; i++) {
		logError(0, "%s Skipping erase CX page %d!\n", tag, i);
		fromIDtoMask(i, mask, mask_cnt);
	}

	for (i = flash_panel_start_page;
	     i <= flash_panel_end_page && keep_cx >= SKIP_PANEL_INIT; i++) {
