...
static int xiaomi_touch_dev_mmap(struct file *file, struct vm_area_struct *vma)
{
	...
	unsigned long pfn =
		((unsigned long)pdata->phy_base + vma->vm_pgoff * 0x1000) >>
		0xc;
	...
	if (remap_pfn_range(vma, start, pfn, size,
			    __pgprot(0x68000000000fc3))) {
		return -EAGAIN;
	} else {
		pr_info("%s remap_pfn_range %u, size:%ld, success\n", __func__,
...
static int xiaomi_touch_probe(struct platform_device *pdev)
{
	...
	pdata->raw_data =
		(unsigned int *)kzalloc(RAW_SIZE, GFP_KERNEL); // flag: 0xdc0
	if (!pdata->raw_data) {
		ret = -ENOMEM;
