...
#include <linux/usb/tcpc/tcpm_pd.h>
...
enum dpm_charging_policy {
	...
	DPM_CHARGING_POLICY_PPS_IC = DPM_CHARGING_POLICY_PPS |
				     DPM_CHARGING_POLICY_IGNORE_MISMATCH_CURR,
};
...
static inline int tcpm_dpm_pd_request(struct tcpc_device *tcpc, int mv, int ma,
				      const struct tcp_dpm_event_cb_data *data)
{
	return TCPM_ERROR_NO_IMPLEMENT;
}
...
tcpm_dpm_pd_request_ex(struct tcpc_device *tcpc, uint8_t pos, uint32_t max,
		       uint32_t oper, const struct tcp_dpm_event_cb_data *data)
{
	return TCPM_ERROR_NO_IMPLEMENT;
}
